...
/*! \fn bool BamAlignment::GetSoftClips(vector<int> &clipSizes,
                                vector<int> &readPositions,
                                vector<int> &genomePositions,
                                bool usePadded) const
    \brief Identifies if an alignment has a soft clip, and if so, identifies the
           sizes of the soft clips, as well as their positions in the read and reference.

    \param clipSizes a vector of the sizes of each soft clip in the alignment.
    \param readPositions a vector of the 0-based read locations of each soft clip in the alignment
    \param genomePositions a vector of the 0-based genome locations of each soft clip in the alignment
    \param usePadded Inserted bases affect reported position. Default is false, so that reported
                     position stays 'sync-ed' with reference coordinates.


    \return true is >= 1 soft clip in the alignment
*/
bool BamAlignment::GetSoftClips(vector<int> &clipSizes,
    ...
    // initialize reference coordinate to starting position
    int refPosition  = Position;
    // track where each CIGAR operation occurred in the read.
    int readPosition = 0;
    // how many soft clips have we seen for this alignment?
    int clipCount    = 0;
    // are we dealing with the first CIGAR op for this alignment?
    bool firstOp     = true;
    ...
    for ( ; cigarIter != cigarEnd; ++cigarIter) {
        ...
        if ( cigarType == Constants::BAM_CIGAR_MATCH_CHAR ||
             cigarType == Constants::BAM_CIGAR_DEL_CHAR ||
             cigarType == Constants::BAM_CIGAR_REFSKIP_CHAR ) // M, D, N
        {
            // matches, deletions and skips affect both the read and the reference
            refPosition  += cigarLength;
            readPosition += cigarLength;
        }
        else if (cigarType == Constants::BAM_CIGAR_INS_CHAR) // I
        {
            // inserts always affect the read position, but only affect the 
            // genome position if we are using padded alignments.
            readPosition += cigarLength;
            if ( usePadded ) readPosition += cigarLength;
        }
        else if (cigarType == Constants::BAM_CIGAR_SOFTCLIP_CHAR) // S
        {
            clipCount++;
            //////////////////////////////////////////////////////////////////////////////
            ...
            clipSizes.push_back(cigarLength);
            readPositions.push_back(readPosition);
            genomePositions.push_back(refPosition);
        }
        firstOp = false;
    }
    // return whether or not any soft clips were seen
    return clipCount > 0;
}
